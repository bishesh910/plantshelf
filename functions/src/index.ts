...
export const adminDeleteUserByEmail = onCall(
  { region: "us-central1", enforceAppCheck: true, memory: "256MiB" },
  async (req) => {
    ...
    if (!userRec)
      return { ok: true, message: "User not found (already deleted?)" };
...
export const adminRevokeSessionsByEmail = onCall(
  { region: "us-central1", enforceAppCheck: true, memory: "256MiB" },
  async (req) => {
    await assertAdmin(req);
    const targetEmail = String(req.data?.email ?? "").trim();
...
export const adminSendPasswordReset = onCall(
  { region: "us-central1", enforceAppCheck: true, memory: "256MiB" },
  async (req) => {
    await assertAdmin(req);
    const targetEmail = String(req.data?.email ?? "").trim();
